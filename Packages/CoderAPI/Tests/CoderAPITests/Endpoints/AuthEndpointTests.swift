...
struct AuthEndpointTests {
    ...
    func fetchAuthMethods() async throws {
        ...
        let methods = try await Fixtures.client(session: stub.session).fetchAuthMethods()
    ...
    func login() async throws {
        ...
        let token = try await Fixtures.client(session: stub.session).login(email: "a@b.com", password: "x")
    ...
    func loginUnauthorized() async throws {
        ...
        await #expect(throws: CoderAPIError.unauthorized(message: "Invalid credentials")) {
            _ = try await Fixtures.client(session: stub.session).login(email: "a@b.com", password: "x")
        }
    }
